...
#set document(
  title: "Privilege on the Exit Path: A Name-Agnostic Static Detector and a Tiered Benchmark for Malicious Solidity Contracts",
  author: "TRUST404",
  ...
  header: context {
    if counter(page).get().first() > 1 {
      ...
      grid(
        ...
        text(style: "italic")[EarthIsMine on BAYBENCH],
...
#align(center)[
  ...
  #v(0.62em)
  #text(size: 16.2pt, weight: "bold", hyphenate: false)[Privilege on the Exit Path:#linebreak()A Name-Agnostic Static Detector and a Tiered Benchmark for Malicious Solidity Contracts]
  #v(0.48em)
...
#paper-meta((
  ...
  [*Specifications:*], [#link("../specs/baybench.md")[baybench.md], #link("../specs/detector.md")[detector.md]],
...
#md-table(4, ([Family], [Name], [Representative shapes], [The attacker gains],), (
...
#md-quote[A finding requires a privileged writer, a state variable that writer can set, and an impact: the variable is read in a revert-bearing node on the transfer path, feeds the transferred or fee amount, is the balance or supply ledger itself, or is the destination of an external call on that path.]

#figure(
  image("/docs/research/figures/baybench_01_predicate.png", width: 92%),
  caption: [The three terms are computed independently. A finding needs all three; any two of them describe ordinary code.],
)
...
#figure(
  image("/docs/research/figures/baybench_02_transfer_path.png", width: 92%),
  caption: [The same three terms on a concrete shape. The variable is written where only a privileged caller reaches and read in a revert on the path every holder takes, which is the whole of a family A exit gate.],
)
...
#md-table(4, ([Rule], [Family], [Structural trigger], [Base],), (
    ([`EXIT_​ADDR_​GATE`], [A], [An address-keyed boolean gate read in an end node on the transfer path, on a privileged-writable mapping], [HIGH]),
    ([`BAL_​PRIV_​MINT`], [B], [A privileged credit to the bound ledger or a privileged increase of the supply variable, outside the constructor], [HIGH]),
    ([`LEAK_​ARBITRARY_​TRANSFERFROM`], [C], [A privileged debit at a key that is not sender-dependent, with no allowance read dominating it], [HIGH]),
    ([`OWN_​HIDDEN_​ROLE`], [D], [An authorisation variable that gates a privileged function and is not exposed by any view], [HIGH]),
    ([`STRUCT_​DELEGATECALL_​SETTABLE`], [E], [A `delegatecall` whose target is privileged-writable, or is chosen from a parameter of a privileged function], [HIGH]),
    ([`HONEYPOT_​LEGACY`], [F], [Any of ten structural checks over the HoneyBadger techniques, each on an ETH-exit function], [MED]),
    ([`PONZI_​SHAPE`], [G], [An ETH send to an address loaded from a store populated by a payable function, with no external value source], [MED]),
  ), colspec: (1.72in, 0.92in, 1fr, 1.08in))
...
#figure(
  image("/docs/research/figures/baybench_03_verdict_ladder.png", width: 92%),
  caption: [Bounding discriminators demote a finding to evidence and steer the file toward benign. Governance discriminators only annotate the reason.],
)
...
#figure(
  image("/docs/research/figures/baybench_04_scratch_mirror.png", width: 92%),
  caption: [The retry ladder has to rewrite a source to compile it, and the grader mounts the input read-only. Mirroring the root and rewriting inside the mirror is what keeps a rescued file scoring the same under the grader as it does locally.],
)
...
#figure(
  image("/docs/research/figures/baybench_05_harness.png", width: 92%),
  caption: [The tool is a black box behind one file contract, which is what lets four different implementations be scored identically and offline.],
)
...
#md-table(4, ([Tier], [n], [Provenance], [A failure here is],), (
...
#figure(
  image("/docs/research/figures/baybench_06_scoring.png", width: 92%),
  caption: [A case is scored by how close the verdict is to its label, and the tiers are weighted so that 779 paper fixtures do not decide the headline on their own.],
)
...
#md-table(8, ([Tool], [Weighted], [Mean verdict], [Family recall], [HIGH-FP], [Evidence hit], [Compile fail], [Tier 0 exact],), (
...
#figure(
  ...
  caption: [Tier-weighted score by tool on the 859-file corpus. Weights are 1 for Tier 0, 2 for Tier 1, 1 for Tier 2, 2 for Tier 3.],
)
...
#md-table(5, ([Tool], [Tier 0 (n=5)], [Tier 1 (n=67)], [Tier 2 (n=779)], [Tier 3 (n=8)],), (
...
#figure(
  ...
  caption: [Mean verdict score by tier. The detector is exact on Tiers 0 and 1 and loses its points on the published-corpus tier.],
)
...
#figure(
  ...
  caption: [Mean verdict score by family. Families C, D, E, and G hold between one and eight cases each and are reported for completeness.],
)
...
#figure(
  ...
  caption: [HIGH false-positive rate by tier. The keyword baseline flags every Tier 3 lookalike.],
)
...
#md-table(4, ([Stage], [Tier 2 weighted], [Verdicts on the 527 compiling malicious-labelled files], [Family F recall],), (
...
#md-table(4, ([File], [Label], [Basis for the verdict], [Detector],), (
...
#figure(
  image("/docs/research/figures/baybench_07_same_finding.png", width: 92%),
  caption: [The rule output is identical on both files; only the bounding discriminator differs.],
)
...
#figure(
  image("/docs/research/figures/baybench_08_family_recall.png", width: 92%),
  caption: [Family recall asks only whether one expected family fired. Extra families neither earn credit nor cost any, which is why the metric cannot speak to multi-pattern attribution.],
)
...
#figure(
  ...
  caption: [Distribution of the number of families fired per labelled-malice case.],
)
...
#bib-entry([*\[1\] The Art of the Scam (HoneyBadger)* · Torres, Steichen, State · USENIX Security 2019], [#link("https://www.usenix.org/system/files/sec19-torres.pdf")[PDF], #link("https://arxiv.org/abs/1902.06976")[arXiv]], [First honeypot taxonomy, eight techniques in three levels. Symbolic execution over bytecode; 690 honeypots found, 87 percent precision on manual validation. Appendix Table 5 lists 24 named honeypots with addresses.])

#bib-entry([*\[2\] Pied-Piper* · Ma et al. · ACM TOSEM 2022], [#link("https://dl.acm.org/doi/fullHtml/10.1145/3560264")[ACM], #link("https://github.com/EthereumContractBackdoor/PiedPiperBackdoor")[repository]], [Five ERC-token backdoors, Datalog with directed fuzzing, 189 confirmed in 13,484 contracts, four CVEs, and a released set of 200 backdoor-injected contracts.])

#bib-entry([*\[3\] TokenScope* · Chen et al. · ACM CCS 2019], [#link("https://www4.comp.polyu.edu.hk/%7Ecsxluo/TokenScope.pdf")[PDF]], [Detects inconsistency between storage changes, interface behaviour, and emitted events; precision above 99.9 percent.])

#bib-entry([*\[4\] CRPWarner* · Lin et al. · IEEE TSE 2024], [#link("https://arxiv.org/html/2403.01425")[arXiv], #link("https://github.com/CRPWarner/RugPull")[repository and datasets]], [103 real rug pulls reduced to hidden mint, limiting sell order, and leaking token, expressed as Datalog rules over bytecode. Precision 91.8, recall 85.9, F1 88.7 on 69 open-source scam contracts.])

#bib-entry([*\[5\] Stop Pulling my Rug (Tokeer)* · Zhou, Sun, Ma, Chen, Yan, Jiang · ICSE-SEIP 2024], [#link("http://www.wingtecher.com/themes/WingTecherResearch/assets/papers/Tokeer_CameraReady.pdf")[PDF]], [201 incidents totalling 425 million dollars; four risks with base rates, blacklist 58.7 percent and balance modification 42.3 percent. Models transfer as four sub-processes with five matching plugins; 98.0 recall, 98.9 precision.])

#bib-entry([*\[6\] From Programming Bugs to Multimillion-Dollar Scams (TrapdoorAnalyser)* · Huynh et al. · 2023, revised December 2024], [#link("https://arxiv.org/pdf/2309.04700v4")[arXiv]], [1,859 manually inspected trapdoor tokens, five techniques, semantic checking built on the Slither Python API; dataset of roughly 30 thousand labelled UniswapV2 tokens.])

#bib-entry([*\[7\] Your Token Becomes Worthless (RPHunter)* · Wu et al. · 2025], [#link("https://arxiv.org/html/2506.18398")[arXiv], #link("https://ieee-dataport.org/documents/dataset-reseach-paper-rphunter-unveiling-rug-pull-schemes-crypto-token-code-and")[dataset]], [645 rug pulls with source; code-risk taxonomy of three categories and eight sub-types with counts.])

#bib-entry([*\[8\] Detecting Rug-Pull via balance tracking* · Applied Sciences 2025], [#link("https://www.mdpi.com/2076-3417/15/1/450")[MDPI]], [Tracks per-function balance changes on bytecode across six backdoor types; 0.98 precision, 0.96 recall.])

#bib-entry([*\[9\] Geth-based ERC20 honeypot detector* · Discover Computing 2025], [#link("https://link.springer.com/article/10.1007/s10791-025-09546-w")[Springer]], [Taint analysis over boolean storage indexed by sender or recipient that decides a revert branch; misses the backdoors that never touch the transfer function.])

#bib-entry([*\[10\] SoK: A Taxonomic Analysis of DeFi Rug Pulls* · Sun, Ma, Nie, Liu · ISSTA 2025], [#link("https://dr.ntu.edu.sg/server/api/core/bitstreams/3b63fb18-d311-4bf9-8064-c3b1a39de823/content")[PDF]], [31 papers and 27 industry sources merged into 35 rug-pull types; 13 tools evaluated at 25.7 to 62.9 percent coverage, 9 types detected by none, best tool at 31.3 percent on compound scams. Names four failure modes of static tools.])

#bib-entry([*\[11\] SoK: Rug Pull Causes, Datasets, Tools* · 2024], [#link("https://doi.org/10.48550/arxiv.2403.16082")[arXiv]], [34 root causes and the list of types no evaluated tool covered.])

#bib-entry([*\[12\] ObfProbe: Obfuscated Funds Transfers* · 2025], [#link("https://arxiv.org/html/2505.11320v1")[arXiv]], [Seven bytecode-level obfuscation features over 1.03 million contracts; a state-of-the-art Ponzi detector falls from 79 percent to 12 percent under obfuscation.])

#bib-entry([*\[13\] Static analysis of NFT rug pulls* · 2025], [#link("http://arxiv.org/pdf/2506.07974")[arXiv]], [Slither applied at scale over 49.9 thousand NFT contracts; reports which constructs dominate flagged contracts.])

#bib-entry([*\[14\] Trade or Trick?* · Xia et al. · ACM SIGMETRICS 2022], [#link("https://arxiv.org/pdf/2109.00229")[arXiv]], [10,920 scam tokens, roughly half of Uniswap V2 listings in the studied window.])

#bib-entry([*\[15\] Token Spammers, Rug Pulls, and Sniper Bots* · Cernera et al. · USENIX Security 2023], [#link("https://www.usenix.org/conference/usenixsecurity23/presentation/cernera")[page]], [60 percent of tokens live less than one day; one-day rug pulls estimated at 240 million dollars.])

#bib-entry([*\[16\] Do Not Rug On Me* · Mazorra, Adan, Daza · 2022], [#link("https://arxiv.org/pdf/2201.07220")[arXiv]], [Machine learning over pool, holder, and transaction-graph features across 27,588 tokens.])

#bib-entry([*\[17\] Detecting Ponzi Schemes on Ethereum* · Chen et al. · WWW 2018], [#link("https://user.it.uu.se/~eding810/conferences/WWW18.pdf")[PDF]], [Opcode-frequency and account features with gradient boosting.])

#bib-entry([*\[18\] GoPlus Token Security API* · industry reference], [#link("https://docs.gopluslabs.io/reference/tokensecurityusingget_1")[documentation]], [Boolean token-security fields including honeypot, hidden owner, mintable, modifiable slippage, transfer pausable, blacklist, proxy, external call, and owner-changeable balance.])

#bib-entry([*\[19\] TokenSniffer Exploit Typologies* · industry reference], [#link("https://tokensniffer.readme.io/reference/exploit-typologies")[documentation]], [Exploit typologies and the corresponding tests for proxy, pausable, mint, ownership restoration, maximum transaction amount, modifiable fee, and blacklist.])

#bib-entry([*\[20\] TRUST404 Track 1: Smart Contract Threat Detection, public sample set and grading rules* · organizers · 2026], [#link("https://trust404.co.kr/tracks")[track page]], [Task brief, output schema, boundary rules for the benign and malicious classes, and the discrimination score of plus one for a correct verdict, zero for uncertain, and minus one for a wrong verdict. Vendored in this repository at `public set/challenge_public/` and `docs/judge/challenge_public/`.])

#bib-entry([*\[21\] Slither* · static analysis framework, `slither-analyzer` 0.11.6], [#link("https://github.com/crytic/slither")[repository]], [The parser and intermediate representation this detector is built on, and, run natively, the high-impact baseline in Section 6.])
