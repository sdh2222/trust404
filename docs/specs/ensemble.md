...
| EN-5 | bench parity: `bench run ensemble --repeat 2` (Docker, `--network none`, all tiers) determinism pass; weighted score ≥ detector's on every tier; Tier 1 and Tier 3 HIGH-FP 0; Tier 0 5/5 | measured 2026-09-20 (Run 8, `reports/ensemble/`, image `fca45895dff7`): determinism pass; weighted 0.9756 vs detector 0.9714; Tier 0 5/5; Tier 3 1.0 (≥ 0.9688); HIGH-FP 0 on Tier 1/3; 25 verdict deltas, all `→ Malicious`. **Not met on Tier 1 (0.9859 < 1.0) and Tier 2 (0.8819 < 0.8909)**: 14 deltas land on `Uncertain`-only labels (13 non-compiling Tier 2 fixtures + `STRUCT_EXTERNAL_GATE/mal`) whose source truth is malicious — label decision recorded in `docs/bench/misses.md` Run 8; OPEN until the owner amends or declines |
