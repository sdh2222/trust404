...
| EN-5 | bench parity: `bench run ensemble --repeat 2` (Docker, `--network none`, all tiers) determinism pass; weighted score ≥ detector's on every tier; Tier 1 and Tier 3 HIGH-FP 0; Tier 0 5/5 | OPEN (orchestrator measures after PR C) |
| EN-6 | image: `trust404/ensemble` runs as any uid, on a read-only rootfs, with `--network none`, both engines alive (no `DEGRADED`) | OPEN (PR C) |
