...
| EN-5 | bench parity: `bench run ensemble --repeat 2` (Docker, `--network none`, all tiers) determinism pass; Tier 0 5/5; Tier 1 and Tier 3 HIGH-FP 0; weighted score and every tier mean ≥ detector's **on labels that admit a decisive answer** — the 250 Tier 2 `non-compiling paper fixture` labels stay Uncertain-only by owner decision (2026-09-20), so Tier 2 is compared on the 529 Malicious-labelled files (Malicious / Benign-miss / Uncertain counts) | owner amend 2026-09-20 (Henry); **met** — Run 9 (`reports/ensemble/`, `reports/detector/`; images `f888fd9077c8` / `cd5539f6b9de` from tree `a53c690`, Docker `--network none --repeat 2`, all tiers): determinism pass; Tier 0 5/5; HIGH-FP 0 on Tier 1/3; weighted 0.9803 vs detector 0.9703; Tier 1 1.0 vs 0.9965; Tier 3 1.0 vs 0.9688; Tier 2 on the 529 Malicious-labelled files Malicious 513 / Benign-miss 16 / Uncertain 0 (mean 0.9698) vs 503 / 18 / 8 (0.9584). The full Tier 2 mean (0.8819 vs 0.8909) reflects the 250 Uncertain-only labels (declined), `docs/bench/misses.md` Run 9 |
